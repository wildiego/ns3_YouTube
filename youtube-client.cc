...
//		 }
		 ...
		 NS_LOG_DEBUG ("[c*c*] Stop TX and close TCP connection for "<< schedChunk);
          ...
          if (m_firstVideo)
            {
        	  NS_LOG_DEBUG (" --> This is the first web page");
        	  requestVideoGap = 0;
              m_firstVideo = false;
          ...
          site.windowSize = 8;
          NS_LOG_DEBUG ("The web gap here " << requestVideoGap);
...
void YoutubeClient::Stadistics ()
{
	NS_LOG_INFO ("Writing statistics ... ");
			  	...
			  	if (m_firstWrite == true )
			  		  	    		...
			  		  	    		m_firstWrite = false;
			  		  	    		os_lte << "UserID\tStartClick\tStartPlay\tEndDL\tEndTime\tPausesNum\tPausesTotal\tMOS\tItag_93\tItag_94\tItag_132\tItag_151";
			  		  	    		os_lte << std::endl;
